...
{
    return m_fileService.ensureUploadDir() && initListenSocket() && initEpoll();
}
...
void ChatServer::run()
{
    ...
    while (true) {
        const int readyCount = epoll_wait(m_epollFd, events.data(), static_cast<int>(events.size()), -1);
        if (readyCount == -1) {
            if (errno == EINTR) {
                continue;
            }
...
bool ChatServer::initEpoll()
{
    ...
    return epoll_ctl(m_epollFd, EPOLL_CTL_ADD, m_listenFd, &event) != -1;
}
